...
class DGNAgent(OffPolicyAgent):
    ...
    def __init__(self, nact, nagent, dgnet, adjnet, lr, reg_coef, start_epsilon, end_epsilon, epsilon_decay, transition_type,
        ...
        self.adjnet = adjnet
        self.target_adjnet = deepcopy(adjnet)
        self.opt_adj = torch.optim.Adam(self.adjnet.parameters(), lr=0.0005)
    ...
    def _td_loss(self, batch):
        ...
        with torch.no_grad():
            ...
            _, next_dist, _ = self.dgnet(next_z, batch.adjs)

        projected_dist = self.project_dist(target_best_dist, batch, self.gamma)

        z = self.dgnet.encode_state(batch.states)
        qvalue_dist, dist, _ = self.dgnet(z, batch.adjs)
    ...
    def update(self):
        ...
        # self.opt_adj.zero_grad()
        # adj_loss = self._adj_loss(batch)
        # adj_loss.backward()
        #
        # if self.grad_clip:
        #     self._clip_grad(self.adjnet.parameters())
        # self.opt_adj.step()
    ...
    def update_target(self):
        ...
        self._update_target(self.adjnet, self.target_adjnet, tau=0.1)
