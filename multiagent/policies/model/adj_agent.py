...
class ADJAgent(OffPolicyAgent):

    def __init__(self, actor, critic1, critic2, lr_actor, lr_critic, gamma, grad_clip,
                 transition_type,
                 buffer_size,
                 batch_size,
                 tau,
        ...
        super(ADJAgent, self).__init__(transition_type, buffer_size, batch_size, tau, gamma, grad_clip, device)
    ...
    def update(self, intrinsic_reward=None):
        ...
        loss_td1, td_error1 = self._td_loss(batch, self.critic1, self.target_critic1, intrinsic_reward)
        loss_td1.backward()
        if self.grad_clip:
        ...
        loss_td2, td_error2 = self._td_loss(batch, self.critic2, self.target_critic2, intrinsic_reward)
        loss_td2.backward()
        if self.grad_clip:
